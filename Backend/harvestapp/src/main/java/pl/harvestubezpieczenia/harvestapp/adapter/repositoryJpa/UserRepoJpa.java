...
public interface UserRepoJpa extends JpaRepository<User, Long> {
    ...
    @Query(value = "select k.id_kalkulacja, k.numer_kalkulacji, k.data_poczatku_ochrony, k.data_konca_ochrony, u2.imie, u2.nazwisko, k.status_kalkulacji from harvest.kalkulacja k left join harvest.uzytkownik u on u.id_uzytkownik = k.id_ubezpieczajacy left join harvest.uzytkownik u2 on u2.id_uzytkownik = k.id_ubezpieczony where u.email = :email or u2.email = :email order by k.id_kalkulacja desc", nativeQuery = true)
